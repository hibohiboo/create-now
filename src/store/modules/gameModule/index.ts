...
// ## コンポーネント
/// １つのゲーム要素の処理が複数のドメイン（ゲームサブシステム）に
/// またがるものであっても、各ドメインが結合しないようにする

// 状態の共有: 全てのオブジェクトが持っていて当然な基本的な属性（位置・大きさ）に使うと便利
// 双方が互いのオブジェクトの参照を持つ： 明らかに別物だが密接に関係しているもの（アニメとレンダリング・入力とAI・物理シミュレーションと衝突検知）
// メッセージ送信： 重要度の低い通信。衝突したときにサウンドコンポーネントに音を出すようにメッセージを送る

// ## イベントキュー
/// 「メッセージやイベントの送信」と「その受信や処理」を時間的に分離する

// ## サービスロケーター
/// サービスを利用するコードに対して、サービスが実装された具象クラスと結合することなしに
/// サービスへのグローバスなアクセスポイントを提供する

// ## データ局所化
/// CPUのキャッシングを有効活用するデータ配置により、
/// メモリへのアクセスを高速化する

// ## ダーティフラグ
/// 結果が必要となるまで処理を遅らせることで不要な作業を避ける

/// ブラウザ内で変更され、サーバに送信する必要が生じたデータを管理するのにダーティフラグが使用されている。

// ## オブジェクトプール
/// 個々のオブジェクトに対してメモリの割り当てと開放を行うのではなく、
/// プール内にあるオブジェクトを再利用することで、
/// 実行効率とメモリ使用効率を向上させる

// ## 空間分割
/// 物体のインスタンスは、物体の位置によって整理されたデータ構造に保存することで、
/// 効率的に探し出せるようにする
