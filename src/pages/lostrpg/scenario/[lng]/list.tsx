...
const Page: NextPage = () => {
  ...
  return (
    <Container>
      ...
      <h2>{t('lostrpg_scenario_list_title')}</h2>
      <Box mt={2}>
        {vm.authUser ? (
          <Link
            href={`/lostrpg/scenario/[lng]/edit`}
            as={`/lostrpg/scenario/${vm.i18n.activeLocale}/edit`}
          >
            {t('common_create')}
          </Link>
        ) : (
          ...
          <List
            ...
            subheader={
              <ListSubheader component="div" id="nested-list-subheader">
                {t('lostrpg_scenario_list_title')}
              </ListSubheader>
            }
            className={classes.root}
          ></List>
        )}
        {vm.scenarios.map((item) => (
          <ListItemLink
            href={{
            ...
            key={item.id}
            as={`/lostrpg/public/${vm.i18n.activeLocale}/scenario?id=${item.id}`}
          >
