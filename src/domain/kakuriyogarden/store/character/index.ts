...
export const useCharacterViewModel = ()=>{
  ...
  const [profile, setProfile] = useState(sampleCharacter.profile)
  const [hope, setHope] = useState(sampleCharacter.hope)
  const [hopeDetail, setHopeDetail] = useState(sampleCharacter.hopeDetail)
  ...
  useEffect(()=>{
    ...
    setHope(loadedData.hope)
    setHopeDetail(loadedData.hopeDetail)
  },[])
  useEffect(()=>{
    saveData(character)
  }, [character])
